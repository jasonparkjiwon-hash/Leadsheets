  ...
  function exportAllAsPDF(){
    ...
    const css=`
      *{box-sizing:border-box}html,body{margin:0;background:#e9edf2;color:#101820;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
      ...
      @media(max-width:620px){.sheet{padding:20px 18px}.sheet-head{display:block}.orientation{margin-top:12px;justify-content:flex-start}.orientation>div{min-width:66px}.sheet h1{font-size:24px}.chord{font-size:14px}}
      @media print{html,body{background:white}.toolbar{display:none}.book{max-width:none;margin:0;padding:0}.sheet{box-shadow:none;margin:0;padding:0}.sheet.new-page{break-before:page;page-break-before:always}.system{break-inside:avoid}.section-head{break-after:avoid}@page{size:auto;margin:11mm 12mm}}
    `;
    w.document.open();
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Lead Sheets — All charts</title><style>${css}</style></head><body><div class="toolbar"><div><strong>All lead sheets</strong><br><span>${sheets.length} sheet${sheets.length===1?'':'s'} · 4 measures per system</span></div><button onclick="window.print()">Export / Save as PDF</button></div><main class="book">${sheets.map(printSheetHTML).join('')}</main></body></html>`);
    w.document.close();
